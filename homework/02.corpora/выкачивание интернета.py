...
def download_page():
        for k in range (1,765): # переход по страницам с новостями (12 штук на каждой странице)
                textpage = ""
                number = str(k + 1)
                try:
...
def information(textnews, fullurl):
        ...
        metadata(author, name, date, category, fullurl, year, month) # запись метадаты
        clean_t = cleaning(textnews)
        onewordname = name.replace(" ", "_")
        onewordname = re.sub("[.,!?()\"-\/]", "", onewordname )
        making_plain(clean_t, author, onewordname, month, year, date, category, fullurl, name) # запись специального plain текста
        ...
        making_plain1(clean_t, author, onewordname, month, year, date, category, fullurl, name) # запись plain текста
        print("новость готова.")

        
def metadata(author, name, date, category, fullurl, year, month):
        path = "БАМ{}plain{}{}{}{}{}{}.txt".format(os.sep, os.sep, year, os.sep, month, os.sep, name)
        with open (os.path.join('БАМ', 'metadata.csv'), 'a', encoding = 'utf-8') as f:
                output = csv.writer(f, delimiter='\t')
                output.writerow(["\n{}\t{}\t\t\t{}\t{}\tпублицистика\t\t\t{}\t\tнейтральный\tн-возраст\tн-уровень\tрайонная\t{}\tБАМ\t\t{}\tгазета\tРоссия\tАмурская область\tru".format(path, html.unescape(author), html.unescape(name), date, category, fullurl, year)])
...
def making_plain(clean_t, author, onewordname, month, year, date, category, fullurl, name):
...
def making_plain1(clean_t, author, onewordname, month, year, date, category, fullurl, name):
        plain = "@au {}\n@ti {}\n@da {}\n@topic {}\n@url {}\n{}".format(author, name, date, category, fullurl, clean_t)
        path = "БАМ{}plain{}{}{}{}" .format(os.sep,  os.sep, year, os.sep, month)
...
def tagged_xml(month, year, onewordname, name):
        path_i = "{}Users{}margaritaberseneva{}Desktop{}БАМ{}plain{}{}{}{}{}".format(os.sep, os.sep, os.sep, os.sep, os.sep, os.sep, year, os.sep, month, os.sep)
        path_o = "{}Users{}margaritaberseneva{}Desktop{}БАМ{}mystem-xml{}{}{}{}{}".format(os.sep, os.sep, os.sep, os.sep, os.sep, os.sep, year, os.sep, month, os.sep)
        if not os.path.exists(path_o):
                os.makedirs(path_o)
        os.system(("{}Users{}margaritaberseneva{}Downloads{}mystem -idn --format xml {}{}.txt {}{}.xml").format(os.sep, os.sep, os.sep, os.sep, path_i, onewordname, path_o, onewordname))
        print("размеченный xml текст готов.")


def tagged_plain(month, year, onewordname, name):
        path_i = "{}Users{}margaritaberseneva{}Desktop{}БАМ{}plain{}{}{}{}{}".format(os.sep, os.sep, os.sep, os.sep, os.sep, os.sep, year, os.sep, month, os.sep)
        path_o = "{}Users{}margaritaberseneva{}Desktop{}БАМ{}mystem-plain{}{}{}{}{}".format(os.sep, os.sep, os.sep, os.sep, os.sep, os.sep, year, os.sep, month, os.sep)
        if not os.path.exists(path_o):
                os.makedirs(path_o)
        os.system(("{}Users{}margaritaberseneva{}Downloads{}mystem -idn {}{}.txt {}{}.txt").format(os.sep, os.sep, os.sep, os.sep, path_i, onewordname, path_o, onewordname))
        print("размеченный plain текст готов.")


def main():
        if not os.path.exists("{}Users{}margaritaberseneva{}Desktop{}БАМ".format(os.sep, os.sep, os.sep, os.sep)):
                os.mkdir("БАМ")
        with open (os.path.join('БАМ', 'metadata.csv'), 'a', encoding = 'utf-8') as f:
                output = csv.writer(f, delimiter='\t')
